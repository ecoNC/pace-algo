"""
Crypto regime-routed module — OPTIMIZED (meta-labeling per leg + cutoff sweep).

phase8c proved the routed module is all-years-positive (2026 strongest) at PF ~1.13.
Now apply the FX-winning levers: META re-rank on each leg's candidates + a top-N cutoff
sweep. Caches the augmented (Rtf/Rmr/regime) pool to disk for speed.

Walk-forward, per-year + per-fold robustness, cost {0.02, 0.05} ATR, top-N {4,6,8}.
If ~1.25-1.3 all-years-positive robustly at realistic cost -> crypto module candidate.

Output: results/model_validation/phase8d_crypto_opt_<UTC>/crypto_opt.json
Run:    python scripts/phase8d_crypto_optimized.py
"""
...
COSTS = [0.02, 0.05]
TOPNS = [4, 6, 8]
CACHE = REPO / "data" / "processed_v2" / "extended" / "crypto_regime_aug.parquet"
...
def get_pool():
    ...
    if CACHE.exists():
        ...
        for col in ["_Rtf", "_Rmr"]:
            pool[col] = aug[col].reindex(pool.index).values
        return pool, feats
...
def main():
    ...
    OUT = REPO / "results" / "model_validation" / f"phase8d_crypto_opt_{stamp}"; OUT.mkdir(parents=True, exist_ok=True)
    pool, feats = get_pool()
    ...
    print(f"pool={len(pool):,}  (cached aug={CACHE.exists()})")
    ...
    for sd in SEEDS:
        for ts0 in FOLD_STARTS:
            ...
            def fitleg(Rcol, top):
                ...
                me = train_lgbm(X(cand, feats), y(cand), X(cand, feats), y(cand), 100, None, sd)
                return (m, me, top, gen)
    ...
    (OUT / "crypto_opt.json").write_text(json.dumps(dict(trendfeat=topTF, mrfeat=topMR, costs=COSTS, topns=TOPNS, results=res), indent=2, default=str), encoding="utf-8")
    print(f"\nDone -> {OUT}")
