...
5. ✅ **Phase C.6 ABGESCHLOSSEN (NB14f-v2 commit `80bad05`):** Pool-Expansion (FX_TRAIN += NZDUSD, FX_HOLDOUT += USDCAD) hat 3/4 Pairs sauber gestaffelt (GBPUSD/AUDUSD/USDCAD), **USDCHF strukturell invertiert** (Conservative 0.17 < Aggressive 0.97). ANN-015 Pass-Kriterien: 1/3 erfüllt. **Aber kein V1-Blocker** — USDCHF wird per ANN-016 als Architektur-Signal interpretiert, nicht als Bug.

6. 🟡 **NEU 2026-05-28 (ANN-016) — STRATEGISCHER PIVOT: FX wird Reference Blueprint, nicht V1-Shortcut.** Nico-Direktive: kein FX-only-V1-Launch, FX vollständig industrialisieren als wiederverwendbarer Blueprint für V2 Crypto/Indices/Commodity. **V1-Launch erst wenn ≥ 2 Asset-Klassen über Blueprint produktionsreif** (ANN-016 Lock 3). [ANN-016 FX as Reference Blueprint](docs/decisions/ANN-016-fx-as-reference-blueprint-industrialization-first.md).

7. 🟡 **NEXT: Phase D — FX-Industrialisierung (D.1–D.8, architecture-first dann technical):**
   - **D.1 USDCHF Deep-Dive (NB15a, IMMEDIATE NEXT):** Session-Problem / Regime-Problem / Liquidity-Problem / Filter-Interaction? Per-Session SHAP, Filter-Impact-Decomposition.
   - **D.2 Universal-vs-Per-Pair Decision (ANN-017 nach D.1):** Filter-Stack universal vs Per-Pair-Overrides vs adaptive Session-Profile. Mit Override-Discipline (ANN-016 Lock 5).
   - **D.3 Pair-/Session-Behavior Map (NB15b)**
   - **D.4 Failure-Case Documentation**
   - **D.5 Pine bit-exact Validation (NB15c)**
   - **D.6 Non-repaint + Live-Bar Validation (NB15d)**
   - **D.7 Router-Integration (NB15e)** — Blueprint-fähiger Pine-Code
   - **D.8 User-Layer Constraint Tests (NB15f)**

8. ⏭️ **Phase E — V2 Multi-Asset (BLOCKED auf Phase D Abschluss):** Crypto / Indices / Commodity über Blueprint. Kein paralleler Bau während Phase D läuft (ANN-016 Lock 1).

9. ⏭️ **R-13 NY-Session-Decomposition** wird Teil von D.1 (USDCHF Deep-Dive).

10. ✅ **R-14 Cutoff-Konvergenz: SUPERSEDED durch ANN-013/14.** Cluster-Detection ersetzt Quantil-basierte Cutoffs.

11. **NICHT** NB16 (Crypto-Modell) bauen vor Phase D Abschluss (ANN-016 Lock 1).
...
| 2026-05-28 | heim-pc | home | **STRATEGISCHER PIVOT — ANN-016 LOCKED: FX as Reference Blueprint (Industrialization-First).** Nico-Direktive: FX wird NICHT V1-Shortcut sondern Core-Blueprint für alle V2-Asset-Klassen. **V1-Launch erst wenn ≥ 2 Asset-Klassen über denselben Blueprint produktionsreif** (Anti-FX-Sonderprojekt-Lock). USDCHF-Bruch wird verstanden, nicht wegoptimiert. Phase D komplett umstrukturiert (architecture-first D.1–D.3, dann technical D.4–D.8). Override-Discipline gelockt (4 Kriterien: statistical/structural/OOS/reproducible). Core Engine vs User Layer Trennung gelockt. NB14f-v2-Daten = Research-Input für D.1, NICHT V1-Lock. Geschrieben: `docs/decisions/ANN-016-fx-as-reference-blueprint-industrialization-first.md` (alle 6 Locks). Aktualisiert: `docs/roadmap.md` (Phase C.6 abgeschlossen, Phase D NEU mit D.1–D.8, Phase E V2 Multi-Asset, Anti-FX-Sonderprojekt-Negativliste), `docs/model_registry.md` (FX-Slot "Industrialization in Progress", V2-Slots gewartet auf Phase E), `docs/pine_router_design.md` (Core/User Layer Sektion verankert), `docs/decisions/README.md` (ANN-016 in Index), `docs/decisions/ANN-005-v1-vs-v1.5-scope-split.md` (Cross-Ref "Scope verschärft durch ANN-016"). HANDOFF Section 16 (Phase C.6 als abgeschlossen, NB15a/D.1 als IMMEDIATE NEXT) + Section 16a (R-21 USDCHF, R-22 Override-Discipline, R-23 Anti-FX-Sonderprojekt) + Section 19 updated. | (dieser HANDOFF-Commit) | **NEXT: NB15a (D.1 USDCHF Deep-Dive) bauen.** Per-Session SHAP, Filter-Impact-Decomposition, Volatility-Regime-Analyse, Vergleich mit USDCAD (was funktioniert) vs USDCHF (was bricht). Output: Strukturbefund-Doku, Input für ANN-017 (D.2 Universal-vs-Per-Pair Decision). |
