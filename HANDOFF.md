...
5. ⏭️ **NEXT: NB15 (Phase D Architecture Validation)** — Pine-Router-V1-Stub auf FX-Modell anwenden, bit-exact Validation Python↔Pine, Pine-Budget-Check, UI-Warning für nicht-5m. Architektur ist gelockt (ANN-009), NB15 ist Validation, nicht mehr Auswahl.

6. ⏭️ **NB13b/c Optional (parallel zu NB15):** Crypto-Spezialmodell trainieren (eigenes LGBM auf Crypto-only Pool), Quality-Anchor-Check. Wenn pass → V2-ready.

7. ⏭️ **R-14 Cutoff-Konvergenz fixen** (UX-blockend für V1): Standard- und High-Tier kollabieren auf identische Cutoffs auf 5m. NB14b-Run nur für Cutoff-Recalibration (logit-transform oder fixed-density). Locked Rule für V1-Release.

8. ⏭️ **R-13 NY-Session-Decomposition** (Marketing-relevant): Per-Session-SHAP-Test ob die 66.6%-Konzentration echter Effekt oder Bid-Side-Datenartefakt ist.

9. **NICHT** NB 09 (Pine Generator) bauen vor NB15 Abschluss + R-14 Fix.
...
| R-17 | **Hardcoded Probability-Werte sind verboten ab 2026-05-28** | hoch (Audit-Lock) | aktiv | Process | Nicos neue Locked Rule nach NB14d. Erlaubt: cluster rank, frequency, stability. Verboten: hartkodierte Probability-Thresholds (`0.4096` etc.), Single-Run-basierte Cutoffs, Percentile-Cutoffs ohne Cluster-Verifikation. Begründung: absolute Werte driften zwischen Modell-Trainings, Cluster-Struktur bleibt stabil. Siehe [ANN-013](docs/decisions/ANN-013-cluster-based-premium-detection.md). |
| R-18 | **Multi-Run-Robustheits-Regel** (Nico, 2026-05-28) | hoch (Process) | aktiv | Process | Wichtige Zahlen (Premium PF, Cutoff, Trade-Count, Max-Proba, Sigs/Tag) brauchen MINDESTENS 3 reruns mit mean/std. Single-Run-Entscheidungen sind ab jetzt verboten. NB14e implementiert das als Default-Pattern. |
