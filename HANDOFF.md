...
| 2026-05-28 | arbeits-pc | work | **BUILD 2 — Härtung per Nico-Direktive (4 Änderungen).** Vor NB15c-Run hat Nico 4 Production-Hardening-Locks eingefordert, alle implementiert: (1) **`AUTO_PUSH = False` default** in NB15c S0 — Outputs landen nur im Drive bis Nico Pine-Compile + TV-Live-Test bestanden; dann manuell auf True setzen + S10 re-runnen. Vermeidet "kaputter Export landet direkt im main". (2) **Determinismus-Hashes** in `pine_codegen.py` + snapshot.json: `__version__ = '0.1.0'`, `feature_registry_hash()` (SHA256-Prefix), `used_feature_list_hash()` (order-preserving). Snapshot logged jetzt `hashes.feature_registry_hash`, `hashes.used_feature_list_hash`, `hashes.pine_codegen_version` — spätere Runs können exakt gediffed werden. (3) **Erweiterte Pine-Limits** in `estimate_pine_ops()`: zusätzlich `n_chars`, `function_count` (count of `=>`), plus n_lines / n_bytes / request.security / ta.* / math.* — alle TradingView-Limits getrackt, nicht nur ops/bar. (4) **Soft-Fail** in `render_feature_engine()`: fehlende Pine-Impl wird NICHT zum SystemExit, sondern auto-droppt mit `f_<name> = 0.0  // DROPPED` als Fallback. `engine['dropped_features']` wird in snapshot persistiert + Warning geloggt. Predictions in Pine divergieren leicht von booster für gedroppte Features (Stage B Bit-exact bleibt unaffected — testet booster vs python_reimpl auf FULL Feature-Set). NB15c regeneriert. Tests grün, Smoketest grün. | (next commit) | **Nico in Colab:** NB15c S0-S9 laufen lassen (~15 min). S10 schreibt Outputs nur in Drive (AUTO_PUSH=False). Dann TradingView-Compile + Live-Test. Wenn OK: `AUTO_PUSH = True` in S0, S10 nochmal laufen → push. Wenn nicht: hier Bug-Report → ich fixe → Re-Run. |
