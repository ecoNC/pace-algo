...
7. 🔥 **NEU 2026-05-28 (ANN-018) — FUNDAMENTAL ARCHITECTURE LOCK: 4-Layer Decision-Assisted System.** Nico-Direktive nach NB15b + Swift-Algo-Lessons. Single-Signal-Modell-Annahme überstellt. Neue Zielarchitektur:
   - Layer 1: Core Signal Engine (existing)
   - Layer 2: **Market Regime Dashboard (NEU, PFLICHT)** — Trend/Strength/Range pro TF + Overall Market State
   - Layer 3: **Interaction Layer (NEU, PFLICHT)** — Filter-Combinations empirisch validiert (USDCHF-Pattern als first registry entry)
   - Layer 4: **Backtest + Parameter Transparency (NEU, first-class component)** — "Current vs Backtested Settings"
   
   **ANN-017** entfällt (USDCHF wird in Layer 3 gehandhabt, nicht als isolierte Override-Decision).
   
   V1-Launch-Definition verschärft: ≥ 2 Asset-Klassen über Blueprint UND alle 4 Layer funktionieren.

8. 🟡 **NEXT: Phase D Re-Strukturiert (4-Layer-Industrialization):**
   - **D.2 Multi-Timeframe Market Dashboard (ANN-019 + NB15c) — IMMEDIATE NEXT.** Spec + Validation: welche Trend/Strength/Range-Metriken pro TF, welche TF-Gewichte, welcher Aggregat-Algorithmus. Auf historischen FX-Daten validieren.
   - **D.3 Interaction Layer (ANN-020 + NB15d).** `core/eval/filter_interaction_registry.py` bauen, USDCHF als first entry, Validation-Pipeline für neue Combinations.
   - **D.4 Backtest UI/Logic Spec (ANN-021 + NB15e).** First-class Pine-Component, nicht nachträglich.
   - **D.5–D.9:** Failure-Cases / Pine bit-exact / Non-repaint / Router-Integration / User-Layer-Constraints. Alle verschoben hinter D.2/D.3/D.4 weil Architektur-Layer zuerst stehen müssen.

9. ⏭️ **Phase E — V2 Multi-Asset (BLOCKED auf Phase D Abschluss):** Crypto / Indices / Commodity über alle 4 Layer. Pro Asset-Klasse Multi-TF-Dashboard + eigene Interaction-Registry-Einträge.
...
| 2026-05-28 | heim-pc | home | **FUNDAMENTAL ARCHITECTURE LOCK — ANN-018: 4-Layer Decision-Assisted System.** Nico-Direktive nach NB15b + Swift-Algo-Lessons. Single-Signal-Modell-Annahme überstellt. Neue Architektur: Layer 1 Core Signal Engine + Layer 2 **Market Regime Dashboard (NEU PFLICHT)** + Layer 3 **Interaction Layer (NEU PFLICHT)** + Layer 4 **Backtest+Transparency (NEU first-class)**. PaceAlgo wird "AI Trading Assistant", nicht "AI Trading Signals". Phase D komplett re-strukturiert: D.1 abgeschlossen, D.2 MTF-Dashboard (ANN-019 + NB15c) als IMMEDIATE NEXT, D.3 Interaction Layer (ANN-020), D.4 Backtest-UI (ANN-021), D.5–D.9 technical danach. **ANN-017 entfällt** (USDCHF wird Layer-3-Entry statt isolierte Override-Decision). V1-Launch verschärft: ≥ 2 Asset-Klassen UND alle 4 Layer funktionieren. Geschrieben: `ANN-018-decision-assisted-architecture-multi-timeframe-dashboard.md` (5 Locks). Aktualisiert: `docs/roadmap.md` (komplette Phase-D-Re-Strukturierung mit D.2 als next), `docs/pine_router_design.md` (4-Layer-Spec verankert), `docs/decisions/README.md` (ANN-018 Index). HANDOFF Section 16 (D.1 abgeschlossen, D.2 Multi-TF-Dashboard als IMMEDIATE NEXT) + 16a (R-24 Interaction-aware, R-25 4-Layer-Pflicht, R-26 Pine-request.security-Budget) + 19 updated. | (dieser HANDOFF-Commit) | **NEXT: ANN-019 Multi-Timeframe Market Dashboard Spec schreiben + NB15c bauen.** Spec: welche Trend/Strength/Range-Metriken, welche TF-Gewichte, welcher Aggregat-Algorithmus. NB15c validiert auf historischen FX-Daten. Output: `core/market_regime/`-Modul (klassen-agnostisch designed). |
